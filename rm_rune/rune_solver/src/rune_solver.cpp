...
Eigen::Matrix4d RuneSolver::solvePose(const rm_interfaces::msg::RuneTarget &predicted_target) {
  ...
  if (pnp_solver && pnp_solver->solvePnP(image_points, rvec, tvec, "rune")) {
    // Get the transformation matrix from rune to odom_vision
    try {
      ...
      // Transform to odom_vision
      ps = tf2_buffer_->transform(ps, "odom_vision");
      ...
      FYT_ERROR("rune_solver", "rune to odom_vision error: {}", ex.what());
      throw ex;
    }
  } else {
...
rm_interfaces::msg::GimbalCmd RuneSolver::solveGimbalCmd(const Eigen::Vector3d &target) {
  ...
  try {
    auto gimbal_tf = tf2_buffer_->lookupTransform("odom_vision", "gimbal_link", tf2::TimePointZero);
...
Eigen::Vector3d RuneSolver::getTargetPosition(double angle_diff) const {
  ...
  // and the fact that the position of the Rune are static in the odom_vision frame,
  ...
  // Transform to odom_vision frame
